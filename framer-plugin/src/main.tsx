...
export function AltTextGenerator() {
    ...
    const analyzePage = async () => {
        ...
        try {
            ...
            const nodes = await framer.getCurrentPageNodes()
            const imageInfos: ImageInfo[] = []
            
            for (const node of nodes) {
                if (node.type === "Image" || node.__class === "ImageNode") {
                    try {
                        // Get image URL
                        const url = await node.getImageUrl?.() || node.image || ""
                        
                        // Get current alt text
                        const currentAlt = await node.getAltText?.() || 
                                         node.altText || 
                                         await node.getPluginData?.("altText") || 
                                         null
    ...
    const applyAltText = async () => {
        ...
        try {
            for (const image of images) {
                ...
                try {
                    // Get the node
                    const node = await framer.getNode(image.nodeId)
                    
                    if (node) {
                        // Try multiple methods to set alt text
                        if (node.setAltText) {
                            await node.setAltText(image.generatedAltText)
                        } else if (node.setPluginData) {
                            await node.setPluginData("altText", image.generatedAltText)
                        } else {
                            // Try to update the node properties directly
                            await framer.setAttributes(image.nodeId, {
                                altText: image.generatedAltText
                            })
                        }
                        successCount++
                    }
    ...
    return (
        <div style={{ padding: "20px", fontFamily: "Inter, sans-serif", height: "100%", overflowY: "auto" }}>
                    ...
                    <div style={{ display: "flex", gap: "10px" }}>
                        <button
                            ...
                            style={{
                                flex: 1,
                                padding: "10px",
                                backgroundColor: (!apiKey || selectedImages.size === 0 || isGenerating) ? "#ccc" : "#4caf50",
                                color: "white",
                                border: "none",
                                borderRadius: "4px",
                                cursor: (!apiKey || selectedImages.size === 0 || isGenerating) ? "not-allowed" : "pointer"
                            }}
                        >
                        ...
                        <button
                            ...
                            style={{
                                flex: 1,
                                padding: "10px",
                                backgroundColor: (images.filter(img => img.generatedAltText).length === 0 || isApplying) ? "#ccc" : "#ff9800",
                                color: "white",
                                border: "none",
                                borderRadius: "4px",
                                cursor: (images.filter(img => img.generatedAltText).length === 0 || isApplying) ? "not-allowed" : "pointer"
                            }}
                        >
            ...
            <div style={{
                ...
                <ol style={{ marginLeft: "20px", lineHeight: "1.6" }}>
                    <li>Enter your OpenAI API key and save it</li>
                    <li>Click "Find Images Without Alt Text" to scan the current page</li>
                    <li>Select the images you want to generate alt text for</li>
                    <li>Click "Generate Alt Text" to create descriptions</li>
                    <li>Review the generated text</li>
                    <li>Click "Apply Alt Text" to update your Framer project</li>
                    <li>Publish your project to make changes live</li>
                </ol>
            </div>
        </div>
    )
